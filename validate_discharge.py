...
| Strict   | ≥5/5 | 81% | **96%** |
    ...
    # ─────────────────────────────────────────────────────────────────
    # Helper: parse external TXT annotation file
    # ─────────────────────────────────────────────────────────────────
    def _parse_txt_annotations(txt_content: str) -> list:
        """
        Parse a TXT annotation file. Supports common clinical formats:
          • One timestamp per line: 12.345 or 0:12.345 or 00:00:12.345
          • Tab/comma/space separated: onset [tab] duration [tab] label
        ...
        for line in txt_content.strip().splitlines():
            ...
            if not parts:
                continue
            # Parse first token as time
            time_str = parts[0]
            onset_sec = None
            # HH:MM:SS.mmm or MM:SS.mmm or SS.mmm
            hms = re.match(r"^(\d+):(\d+):(\d+\.?\d*)$", time_str)
            ms  = re.match(r"^(\d+):(\d+\.?\d*)$", time_str)
            sec = re.match(r"^(\d+\.?\d*)$", time_str)
            if hms:
                h, m, s = hms.groups()
                onset_sec = int(h)*3600 + int(m)*60 + float(s)
            elif ms:
                m, s = ms.groups()
                onset_sec = int(m)*60 + float(s)
            elif sec:
                onset_sec = float(sec.group(1))
