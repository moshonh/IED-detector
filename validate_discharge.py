...
| Strict   | ≥5/5 | 81% | **96%** |
    ...
    with tab_upload:
        uploaded = st.file_uploader(
            "CSV format: first column = channel names (Fp1…O2), "
            "remaining columns = samples (µV).",
            type=["csv"],
        )
        if uploaded is not None:
            try:
                ...
                ch_names_up = list(df.index)
                eeg_mat_up  = df.values.astype(float)
                valid_chs   = [c for c in ch_names_up if c in ELECTRODE_POS_2D]
                if len(valid_chs) < 3:
                    st.error(f"Need ≥3 valid 10-20 channel names. Found: {valid_chs}")
                else:
