...
| Strict   | ≥5/5 | 81% | **96%** |
    ...
    def _load_edf(file_bytes: bytes):
        """
        Load EDF/EDF+ file using pyedflib.
        Returns (eeg_matrix, ch_names, fs, annotations).

        annotations: list of (onset_sec, duration_sec, description) tuples.
        """
        ...
        try:
            ...
            fs_vals   = [f.getSampleFrequency(i) for i in range(n_ch)]
            fs_out    = float(fs_vals[0])          # use first channel fs
            n_samp    = f.getNSamples()[0]
            eeg_mat   = np.zeros((n_ch, n_samp), dtype=float)
            for i in range(n_ch):
                eeg_mat[i, :] = f.readSignal(i)
            # Read annotations (EDF+ / BDF+)
            annotations = []
            try:
                ann_raw = f.readAnnotations()
                # ann_raw = (onsets_sec, durations_sec, descriptions)
                for onset, dur, desc in zip(*ann_raw):
                    annotations.append((float(onset), float(dur), str(desc).strip()))
            except Exception:
                pass
            f.close()
        finally:
            os.unlink(tmp_path)
        return eeg_mat, ch_names, fs_out, annotations
    ...
    annotations_list = []   # (onset_sec, dur_sec, description)
    ...
    with tab_edf:
        uploaded = st.file_uploader(
            ...
            "Annotations (neurologist markings) are read automatically.",
            type=["edf", "EDF"],
        )
        if uploaded is not None:
            try:
                eeg_mat_up, ch_names_up, fs_up, anns_up = _load_edf(
                    uploaded.read()
                )
                ...
                if len(valid_chs) < 1:
                    st.warning(
                        ...
                        f"Channels found: {ch_names_up[:10]}. "
                        f"C6 field analysis will be skipped."
                    )
                    valid_chs = ch_names_up   # use all channels anyway

                n_samp_up = eeg_mat_up.shape[1]

                # ── Show file info ────────────────────────────────────
                col_i1, col_i2, col_i3 = st.columns(3)
                col_i1.metric("Channels",   len(ch_names_up))
                col_i2.metric("Fs (Hz)",    f"{fs_up:.0f}")
                col_i3.metric("Duration",   f"{n_samp_up/fs_up:.1f} s")

                # ── Annotation selector ───────────────────────────────
                if anns_up:
                    st.success(f"✅ Found {len(anns_up)} annotation(s) in the EDF file")
                    ann_labels = [
                        f"[{i+1}]  {onset:.2f}s — {desc or '(no label)'}"
                        for i, (onset, dur, desc) in enumerate(anns_up)
                    ]
                    ann_labels.append("── Manual position ──")
                    chosen_ann = st.selectbox(
                        "Select annotation (neurologist marking):",
                        ann_labels
                    )
                    if chosen_ann == "── Manual position ──":
                        ann_sec = st.slider("Manual annotation (seconds)",
                                            0.5, n_samp_up/fs_up - 0.5,
                                            n_samp_up/fs_up/2, 0.01)
                        ann_idx_up = int(ann_sec * fs_up)
                    else:
                        ann_sel    = ann_labels.index(chosen_ann)
                        ann_idx_up = int(anns_up[ann_sel][0] * fs_up)
                        st.caption(
                            f"Annotation: '{anns_up[ann_sel][2]}' "
                            f"@ {anns_up[ann_sel][0]:.3f}s "
                            f"(sample {ann_idx_up})"
                        )
                else:
                    st.info("No annotations found in EDF. Use manual slider.")
                    ann_sec    = st.slider("Annotation position (seconds)",
                                           0.5, n_samp_up/fs_up - 0.5,
                                           n_samp_up/fs_up/2, 0.01)
                    ann_idx_up = int(ann_sec * fs_up)

                # ── Focus channel selector ────────────────────────────
                focus_up = st.selectbox(
                    "Focus channel (annotated channel):",
                    valid_chs,
                    help="The channel where the discharge was marked."
                )

                # Clamp ann_idx to valid range
                half_w     = int(0.25 * fs_up)
                ann_idx_up = int(np.clip(ann_idx_up, half_w,
                                         n_samp_up - half_w - 1))
                ...
                source_label      = f"{uploaded.name} · {focus_up} · ann@{ann_idx/fs_up:.2f}s"
                annotations_list  = anns_up

            except ImportError:
                st.error("pyedflib not installed. Run: pip install pyedflib then restart.")
